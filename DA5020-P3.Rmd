...
library(corrplot)
cor(df$tip_amount, ~.)
corrplot(cor(df[,seq(7,12)]), diag = FALSE, type="upper",addCoef.col = "black")
```
...
# we think that to have 80% of the data for training purpose so we have enough data for training a good model. in a same time 20% for test is good number of tests and we can have a good evaluation as the data has total 398632 observations. 
```
